...
export default function CurrentlyScrapingTable({jobs}: {jobs: JobType[]}) {
  ...
  const formateDateTime = (inputDate: string) => {
  ...
  const renderCell = React.useCallback((job: JobType, columnKey: React.Key) => {
    ...
    switch (columnKey) {
        case "url":
            return <Link href={cellValue} target="_blank" className="text-primary-500">{cellValue}</Link>
        case "jobType":
            return cellValue.type;
        case "createdAt": 
            return formateDateTime(cellValue);
        case "status" :
            return <Chip className="capitalize" color={statusColorMap[job.status]} size="sm" variant="flat">{cellValue}</Chip>
      default:
        return cellValue;
    }
  }, []);
